...
const validateEmail = (email) => {
  let regex = /^\w+([.]?\w+)@\w+([.]?\w+)(?:.com|.net|.org)\b/g;
...
const findTagNames = elements => elements.map((element) => element.match(/\/\w+/g)).reduce((accumulator, currentValue) => accumulator.concat(currentValue, []));
