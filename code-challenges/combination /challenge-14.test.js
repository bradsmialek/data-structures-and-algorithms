...
// let biggerThanLuke = (data) => data.filter((profile) => profile.mass > 77).reduce((acc, curr) => `${acc.name} - ${curr.name}`)

// let biggerThanLuke = (data) => data.filter((profile) => profile.mass > 77).map((acc) => `${acc.name} `).join('- ')

let biggerThanLuke = (data) => data.filter((profile) => profile.mass > 77).reduce((acc, curr, index) => index === 0 ? acc + curr.name : acc + ` - ${curr.name}`, '');
